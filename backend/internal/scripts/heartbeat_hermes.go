...
ensure_hermes_stack() {
    ...
    if [ ! -S /run/tardi-host-admin/admin.sock ] || [ ! -x /opt/hermes/host-admin/bin/tardi-host-admin ] || [ ! -x /opt/hermes/host-admin/bin/sudo ] || ! grep -q 'desktop.open' /opt/hermes/host-admin/bin/tardi-host-admin 2>/dev/null || ! grep -q 'TARDI_HOST_ADMIN_CLIENT_VERSION=2026070101' /opt/hermes/host-admin/bin/tardi-host-admin 2>/dev/null; then
