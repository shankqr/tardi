...
- **Production is fully torn down** (as of 2026-07-28). All billable resources in `tardi-prod-2026` were deleted: Cloud Run service and job, Cloud SQL instance and backups, Cloud Scheduler, Artifact Registry, Secret Manager secrets, monitoring resources, BigQuery billing dataset, and the Terraform state bucket. The project is unlinked from billing (`billingEnabled: false`). Direct VPC address reservations can take 1–2 hours to disappear after Cloud Run deletion; they and the remaining empty VPC objects are non-billable.
- **Do not deploy prod while the project is paused.** The normal automatic prod deploy rule above is suspended until the production bringup procedure has been completed. See [docs/prod-wind-down.md](docs/prod-wind-down.md).
