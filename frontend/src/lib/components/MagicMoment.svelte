...
<div class="rounded-xl border border-gray-200 p-5">
		...
		<div class="mt-4 space-y-3">
			{#each prompts as item, index}
				<div
					class="group rounded-lg border border-gray-100 p-3 transition-all hover:border-gray-300"
				>
					<div class="flex items-start justify-between gap-3">
						<div class="min-w-0 flex-1">
							...
							<p class="mt-1 text-xs text-gray-500 pl-7">{item.description}</p>
						</div>
						<button
							onclick={() => copyPrompt(index)}
							class="shrink-0 rounded-md border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-900"
						>
