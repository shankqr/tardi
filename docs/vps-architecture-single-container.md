...
│    custom_caddy  │  ← if set, overrides auto-generated Caddyfile
│  }               │
└──────────────────┘
```
...
systemd timer fires
    ...
    heartbeat.sh
         ...
         └──▶ Drift Guards (run every time):
              ...
              │     └── If custom_caddyfile set in DB, use it verbatim
              │         instead of auto-generated routing config
...
### Custom Caddyfile (DB override)

When `custom_caddyfile` is set on a `vps_instances` row, the heartbeat drift guard
uses it **verbatim** instead of auto-generating the default config above.

Use case: instances running user-built web apps with custom domains, TLS certs,
or non-standard port routing (e.g., HTTPS with self-signed certs for Cloudflare
Full SSL mode, API proxy on a different port).

Example (custom domain with TLS + API proxy):
```
b15m.clawmyway.com {
    tls /opt/openclaw/ssl/b15m.crt /opt/openclaw/ssl/b15m.key
...
Set via direct DB update:
```sql
UPDATE vps_instances SET custom_caddyfile = '...' WHERE id = '<uuid>';
```

The heartbeat picks up the value from the API response and applies it on the
next 5-minute cycle. No frontend UI for this — admin-only via SQL.
...
| Aspect | Phase 1 (2 containers) | Phase 2 (iptables NAT) | Phase 3 (Caddy routing) | Phase 3b (Custom Caddy) |
|--------|----------------------|----------------------|------------------------|------------------------|
| Port 80 routing | Caddy Docker container | iptables NAT → 18789 | **Caddy host binary** | Caddy host binary |
| Preview domain | Not functional | Routed to OpenClaw (broken) | **Routes to port 3000** | Custom routing |
| Hostname routing | None | None | **By Host header** | By Host header |
| Docker networking | Bridge (openclaw-net) | Host (--network=host) | Host (--network=host) | Host (--network=host) |
| TLS termination | Caddy (Let's Encrypt) | Cloudflare Proxy (edge) | Cloudflare Proxy (edge) | **Caddy (self-signed) or CF** |
| Caddy runs as | Docker container | Not present | **Systemd service (host)** | Systemd service (host) |
| Caddyfile source | Static | N/A | **Auto-generated by heartbeat** | **DB `custom_caddyfile` column** |
| Backend RPC | `wss://` through Caddy | `ws://:18789` direct | `ws://:18789` direct | `ws://:18789` direct |
| Cert management | Caddy auto-renewal | None (Cloudflare) | None (Cloudflare) | **Per-instance (manual certs)** |
| DDoS protection | None | Cloudflare (free tier) | Cloudflare (free tier) | Cloudflare (free tier) |

## Key Source Files

- `backend/internal/jobs/provisioner.go` — Cloud-init template: installs Caddy, writes Caddyfile, starts systemd service
- `backend/internal/scripts/heartbeat.go` — Caddy drift guard: ensures binary, Caddyfile, service are correct; supports `custom_caddyfile` override; removes old iptables NAT
- `backend/internal/api/agent.go` — Heartbeat API: returns `preview_domain` and `custom_caddyfile` so heartbeat can configure Caddy
...
- `backend/migrations/034_add_custom_caddyfile.sql` — Migration adding `custom_caddyfile` column to `vps_instances`
